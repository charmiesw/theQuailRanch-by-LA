...
public class RanchFormController {
    ...
    private void loadAllRanches() {
        tblRanch.getItems().clear();
        try {
            List<RanchDto> dtoList = ranchBO.getAllRanches();

            for(RanchDto dto : dtoList) {
                tblRanch.getItems().add(
                        new RanchTm (
                                dto.getId(),
                                dto.getDate(),
                                dto.getCategory(),
    ...
    private void generateNextRanchId() {
        try {
            String id = ranchBO.generateNextRanchId();
            txtId.setText(id);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    void btnSaveOnAction(ActionEvent event) {
        ...
        if (isRanchValidated) {
            try {
                boolean isSaved = ranchBO.saveRanch(new RanchDto(id, date, category, amountOfBirds));
    ...
    void btnUpdateOnAction(ActionEvent event) {
        ...
        if (isRanchValidated) {
            try {
                boolean isUpdated = ranchBO.updateRanch(new RanchDto(id, date, category, amountOfBirds));
                if(isUpdated) {
    ...
    void txtSearchOnAction(ActionEvent event) {
        ...
        try {
            RanchDto dto = ranchBO.searchRanch(id);
    ...
    void btnDeleteOnAction(ActionEvent event) {
        ...
        try {
            boolean isDeleted = ranchBO.deleteRanch(id);
